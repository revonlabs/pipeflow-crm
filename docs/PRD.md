...
Soluções como HubSpot e Pipedrive existem, mas são caras ou complexas demais para quem está começando. E mesmo assim essas soluções não oferecem a personalização necessária para cada tipo de cliente — elas são ferramentas genéricas.
...
- Sessão persistente com middleware de proteção de rotas
...
- Busca global por lead/contato/negócio
- Filtros por status, responsável, data de criação, etapa do pipeline

### Onboarding do Usuário

- Fluxo guiado na primeira entrada: criar workspace → convidar time → criar primeiro lead
...
- API pública REST para leitura/escrita de leads (futura)
- Webhooks de eventos (lead criado, deal movido)
...
## 7. Milestones de Desenvolvimento

| # | Milestone | Entregável |
|---|---|---|
| 1 | **Project Scaffold** | create-next-app, Tailwind, shadcn/ui, Supabase client, envs |
| 2 | **Auth** | Login, registro, magic link, middleware de proteção |
| 3 | **Multi-workspace** | Criar workspace, convite por e-mail, papéis Admin/Membro |
| 4 | **Leads & Contatos** | CRUD, listagem com filtros, detail page + timeline |
| 5 | **Pipeline Kanban** | Board DnD Kit, deal cards, persistência no banco |
| 6 | **Dashboard** | Metric cards, gráfico de funil (Recharts), deals próximos |
| 7 | **Monetização** | Stripe Checkout, webhooks, Customer Portal |
| 8 | **Landing Page** | Página pública hero + features + pricing + CTA |
| 9 | **Polish & Deploy** | Skeletons, error boundaries, RLS audit, deploy Vercel |
